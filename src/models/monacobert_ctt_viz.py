...
class MonotonicConvolutionalMultiheadAttention(nn.Module):
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        conv_kernel_layer_out = torch.softmax(conv_kernel_layer, dim=1)
        ...
        # matmul(q X k, v)
        conv_out_layer = torch.matmul(conv_out_layer, conv_kernel_layer_out)
        ...
        attention_probs_out = nn.functional.softmax(attention_scores, dim=-1)
        # |attention_probs| = (bs, n_attn_head, n, n) = (64, 8, 100, 100)
        attention_probs = self.dropout(attention_probs_out)
        # |attention_probs| = (bs, n_attn_head, n, n) = (64, 8, 100, 100)
        ...
        # |outputs| = (bs, n, hs)
        return attention_probs_out, conv_kernel_layer_out, outputs
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        attn_score, conv_score, z = self.attn(Q=z,
                                            K=z,
                                            V=z, 
                                            mask=mask)
        z = x + self.attn_dropout(z)
        # |z| = (bs, n, hs)
        self.attn_scores.append(attn_score.cpu())
        self.conv_scores.append(conv_score.cpu())
...
class MonaCoBERT_CTT_VIZ(nn.Module):
